...
Admin routes follow the same `requireAdmin(req, permission)` pattern per-route, and `/api/account/*` routes follow the same `requireCustomer()` pattern — both, as of 2026-09-21, are additionally backstopped by `src/proxy.ts` (Next's "Proxy," formerly `middleware.ts` — see D-8 in `docs/DECISIONS.md`): every request under `/admin/*`, `/api/admin/*`, or `/api/account/*` is checked for a validly signed, unexpired session token of the right scope *before* it reaches route code, independent of whether that route remembered to call `requireAdmin()`/`requireCustomer()` itself. This closes the "a future route forgets the check" failure mode down to "logged out," not "wide open." It does **not** check fine-grained admin permissions (`MANAGE_LEADS` etc.) or the customer-row DB lookup `requireCustomer()` does — those stay exactly where they were; the proxy only checks signature/scope/expiry, to stay fast and dependency-free. Verified live this session in two passes: admin (6 API routes + the dashboard page blocked with no cookie, a forged cookie rejected cleanly, 5 pre-auth admin endpoints still reachable, a real login → authenticated call → page load worked — 17/17 checks) and customer (6 `/api/account/*` routes blocked with no cookie, all 6 pre-auth endpoints still reachable, a real signup+login worked, admin/customer sessions confirmed mutually rejected on each other's endpoints, the public site unaffected — 23/23 checks).
