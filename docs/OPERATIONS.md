...
- **Production target**: Vercel. **Not confirmed deployed as of this audit** — `vercel.json` and the Blob/Cron wiring assume it, but no evidence exists in-repo that a Vercel project has actually been created and linked. This is a real open item for the owner, not something to assume.
...
**None, currently.** No Sentry, no APM, no uptime check, no log aggregation beyond whatever Vercel's own function logs provide by default. `AuditLog` and `AdminNotification` give an in-app activity trail for admin-driven changes, but nothing alerts a human if, say, the automation cron silently stops firing, a webhook starts failing, or error rates spike. This is a real gap for a production business system — flagged as a priority in `docs/IMPLEMENTATION_PLAN.md`.
