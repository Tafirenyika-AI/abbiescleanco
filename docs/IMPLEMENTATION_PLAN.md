...
**Status: levels 1 & 2 done for admin (2026-09-22, `2c55204`), level 3 and a customer-facing version not started.** `src/lib/server/assistant.ts` + `POST /api/admin/assistant`, surfaced as the "Ask Abbie Assistant" bar in the admin header. Level 1 (§4: typed route registry + deterministic intent matching) reuses `allNavItems` from `src/lib/admin/nav.ts` as the route registry — no new registry needed, it already had `href`/`label`/`description`/`permission`. Level 2 (retrieve/explain authorized data through typed server tools) is 7 real tools (new leads, pipeline value, conversion rate, net revenue, overdue payments, bookings this week/today, unassigned bookings), each a thin wrapper around an existing store function, authorized through the same `requireAdmin()` every other admin route uses. No model call anywhere in this — deterministic keyword matching only, per the blueprint's own §6 instruction not to spend an AI call on navigation or arithmetic a database query already answers. Live-verified: 11/11 checks (auth required, all 7 tools return correct answers against real data, an unmatched query returns an honest "I don't know" instead of guessing).
- **[code]** Level 3 (propose/execute actions after confirmation) — real net-new scope needing careful per-action confirmation UX; not started.
- **[code]** A customer-facing version (§4: "guest can access public services and create inquiry, authenticated customers see only own records") — the admin version's pattern (typed tools + `requireX()` authorization) is directly reusable, just needs its own tool set scoped to `requireCustomer()`; not started.
- **[decision/creds]** A real model-backed layer (natural-language queries beyond the fixed keyword set) needs the Anthropic key already flagged elsewhere in this plan, plus a decision on how much free-text flexibility to allow before it's worth the cost/complexity.
