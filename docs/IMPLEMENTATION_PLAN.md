...
- ~~**[decision]** Whether to build a real `Invoice`/`InvoiceLine` model + PDF generation, or integrate with QuickBooks/similar~~ **Partially resolved 2026-09-23** (`91929bb`) — built a real, simple `Invoice` model (no line-item duplication; always reads live from the linked Quote) with a formal letterhead-style printable/PDF-via-browser-print view, `/admin/invoices`. This answers the "formal invoice document" half of the decision without pre-committing to the bigger question. **Still open**: whether to go further into a full accounting sync (QuickBooks/Xero) or a home-grown double-entry ledger — that larger decision is unchanged and still the owner's to make.
- **[code, once the bigger accounting decision above is made]** Receivables aging report, distinct "billed vs. collected vs. accounting profit" views (blueprint §3.6 explicitly asks these be distinguished — currently `Payment.status=PAID` sums are the only revenue view that exists, via `/admin/reports`).
...
6. **[decision]** Invoicing/accounting approach (Phase 3 — basic printable invoices now done 2026-09-23, `91929bb`; the bigger QuickBooks-vs-ledger question remains open) and workforce/subcontractor classification (Phase 4) — both need the owner's input before more code should be written in those areas, and both gate real further scope.
