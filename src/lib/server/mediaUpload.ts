...
export type DetectedMedia = { kind: "IMAGE" | "VIDEO"; mime: string; ext: string };
...
/**
 * Validates and stores a customer-provided photo or video. Images are re-encoded (auto-rotated,
 * downscaled, and stripped of EXIF -- including GPS coordinates of the customer's home).
 */
export async function saveCustomerMedia(file: File): Promise<SavedMedia> {
  ...
  if (file.size > MAX_VIDEO_BYTES) throw new MediaError("File is too large (max 60MB for video, 12MB for photos).");
  const buf = Buffer.from(await file.arrayBuffer());
  const detected = sniffMedia(buf);
  if (!detected) throw new MediaError("Unsupported file. Please upload a JPG, PNG, WEBP or HEIC photo, or an MP4, MOV or WEBM video.");
  ...
  return { url: await storeBuffer(buf, detected.ext, detected.mime), kind: "VIDEO", mimeType: detected.mime, sizeBytes: buf.length };
}
...
export function isTrustedMediaUrl(url: string): boolean {
  if (/^\/api\/uploads\/client\/[a-f0-9]{32}\.(jpg|png|webp|mp4|mov|webm)$/.test(url)) return true;
  try {
    const u = new URL(url);
