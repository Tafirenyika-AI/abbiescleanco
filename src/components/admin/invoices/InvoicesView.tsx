  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => setPickerOpen(true)} className="inline-flex items-center gap-1.5 rounded-full bg-admin-teal px-4 py-2 text-sm font-semibold text-white">
      ...
      {invoices.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-xl border border-admin-border bg-admin-card">
          <table className="w-full text-sm">
            ...
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-t border-admin-border hover:bg-admin-bg">
...
function NewInvoicePicker({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 p-4" onClick={onClose}>
      <div className="max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-admin-card p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
            ...
            {quotes.map((q) => (
              <li key={q.id} className="flex items-center justify-between rounded-lg border border-admin-border px-3.5 py-2.5">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full bg-admin-teal px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
                >
