...
import { listBankTransactions, getBankSummary } from "@/lib/server/bankStore";
import BankingManager from "@/components/admin/finance/BankingManager";
...
export default async function AdminFinanceBankingPage() {
  ...
  const [transactions, summary] = await Promise.all([listBankTransactions("all"), getBankSummary()]);

  return <BankingManager initialTransactions={transactions} summary={summary} />;
}
